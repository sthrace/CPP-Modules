...
{
	std::cout << BOLDMAGENTA << "A NoName ScavTrap has been created" << DEFAULT << std::endl;
}

ScavTrap::ScavTrap(std::string const name) : ClapTrap(name, 100, 50, 20)
{
	std::cout << BOLDMAGENTA << "ScavTrap " << m_name << " has been created" << DEFAULT << std::endl;
}
...
{
	std::cout << BOLDRED << "ScavTrap " << m_name << " died" << DEFAULT << std::endl;
}
...
{
	std::cout << BOLDCYAN << "ScavTrap " << m_name << " has entered in GateKeeper mode\n";
	std::cout << DEFAULT;
}